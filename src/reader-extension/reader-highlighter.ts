...
import { type KeywordStyle, getCssClasses, resolveKeyword } from 'src/shared';
...
function replaceWithHighlight(node: Node, keyword: KeywordStyle, caseSensitive: boolean) {
  if (
    ...
    if (resolved.type === 'regex') {
      ...
      regexp.lastIndex = 0;
      const match = regexp.exec(node.nodeValue);
      if (match) {
        ...
        const afterText = node.nodeValue.substring(index + matchedText.length);
      ...
      const searchText = resolved.text;
      const index = caseSensitive ? node.nodeValue.indexOf(searchText) : node.nodeValue.toLowerCase().indexOf(searchText.toLowerCase());
      if (index > -1) {
        // parent cannot be null
        const parent = node.parentNode!;
        const beforeText = node.nodeValue.substring(0, index);
        const matchedText = node.nodeValue.substring(index, index + searchText.length);
